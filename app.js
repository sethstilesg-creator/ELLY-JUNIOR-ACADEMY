...
function rubric(x){x=Number(x);return x>=80?"Exceeding Expectation":x>=60?"Meeting Expectation":x>=40?"Approaching Expectation":"Below Expectation"}esc(f.date)}</td><td>${esc(f.adm)}</td><td>${esc(f.student)}</td><td>${money(f.amount)}</td><td>${esc(f.ref)}</td></tr>`).join(""):`<tr><td colspan="5" class="empty">No fee payments recorded.</td></tr>`}</tbody></table></div>`}
